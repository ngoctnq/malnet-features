    ...
    def tokenize(self, inputs, mode="<encoder-only>", max_length=512, padding=False):
        ...
        * `mode`- which mode the sequence will use. i.e. <encoder-only>, <decoder-only>, <encoder-decoder>
        ...
        for x in inputs:
            ...
            # we know that it exceeds max length almost always :(
            # if len(tokens) > max_length:
            #     print(f"Warning: Input sequence length {len(tokens)} is longer than max_length {max_length}.")
...
def get_embedding(model, code_snippet, device):
    ...
    # return tokens_embeddings, func_embedding
    func_embedding = func_embedding.squeeze(0).detach().cpu()
...
def infer_fn(src: str):
    return get_embedding(model, src, device).numpy()
