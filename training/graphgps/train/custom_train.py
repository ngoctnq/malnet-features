        ...
        if is_eval_epoch(cur_epoch):
            ...
            if cfg.metric_best != 'auto':
                ...
                if cfg.wandb.use:
                    ...
                    # log running time and memory
                    bstats["stats/epoch_time"] = full_epoch_times[-1]
                    bstats["stats/epoch_time_avg"] = np.mean(full_epoch_times)
                    bstats["stats/epoch_time_sum"] = np.sum(full_epoch_times)
                    bstats["stats/epoch_time_max"] = np.max(full_epoch_times)
                    bstats["stats/epoch_time_min"] = np.min(full_epoch_times)
                    bstats["stats/epoch_time_std"] = np.std(full_epoch_times)
                    bstats["stats/cuda_memory_allocated"] = torch.cuda.memory_allocated()
                    bstats["stats/cuda_memory_reserved"] = torch.cuda.memory_reserved()
                    bstats["stats/cuda_memory_max_allocated"] = torch.cuda.max_memory_allocated()
                    bstats["stats/cuda_memory_max_reserved"] = torch.cuda.max_memory_reserved()
