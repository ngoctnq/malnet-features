...
from graphgps.loader.dataset.malnet_tiny_features import MalNetTinyFeatures
...
def load_dataset_master(format, name, dataset_dir):
    ...
    """
    if format.startswith('MalNetTinyFeatures'):
        # hard code to keep nodes if '+' in format
        if '+' in format:
        ...
        variant = format.lstrip('MalNetTinyFeatures')

        dataset_dir = osp.join(dataset_dir, format)
        dataset = preformat_MalNetTinyFeatures(dataset_dir, feature_set=name, remove_isolated=remove_isolated, variant=variant)
...
def preformat_MalNetTinyFeatures(dataset_dir, feature_set, remove_isolated=True, variant=''):
    '''
    Feature set has the form:
        collator+ablation+base_feature
    where:
        collator: trim/prune/zero/none
        ablation: base/llm/all
        base_feature: ldp and the likes

    Variant is either '', 'Common', or 'Distinct'.
    '''

    # if '+' in feature_set:
    #     cfc, feature_set = feature_set.split('+')
    # else:
    #     cfc = 'none'
    collator, ablation, base_feature = feature_set.split('+')

    if collator == 'prune' and remove_isolated:
        print(f'Warning: {feature_set} is set with remove_isolated=True!')
        print('This will sometimes lead to empty graphs. Reverting to False.')
        remove_isolated = False

    if base_feature == 'Constant':
        tf = T.Constant()
    # elif base_feature == '_0x5':
    #     tf = T.Compose([T.Constant(value=0)] * 5)
    # elif base_feature == '1x5':
    #     tf = T.Compose([T.Constant(value=1)] * 5)
    # elif base_feature == 'OneHotDegree':
    #     tf = T.OneHotDegree()
    elif base_feature in ['LocalDegreeProfile', 'ldp']:
        tf = T.LocalDegreeProfile()
    elif base_feature == 'none':
        tf = None
    else:
        raise ValueError(f"Unexpected transform function: {base_feature}")

    dataset = MalNetTinyFeatures(collator, ablation, dataset_dir, remove_isolated=remove_isolated, variant=variant)
    dataset.name = 'mntf_' + feature_set
    if not remove_isolated:
        dataset.name += '_keepisolated'

    logging.info(f'Computing "{feature_set}" node features for MalNetTinyFeatures-{variant}.')
    if tf is not None:
        pre_transform_in_memory(dataset, tf)
