...
from utils import reset_args, accuracy, adjust_batch
import random
...
class FeatAgent:
    ...
    def finetune(self, data):
        ...
        assert args.gtrans.debug == 1 or args.gtrans.debug == 2
        model = self.model
        ...
        if args.gtrans.tent:
            ...
            args.gtrans.loss = 'entropy'
            args.gtrans.lr = args.gtrans.lr_feat
            optimizer = optim.Adam(model.bns.parameters(), lr=args.gtrans.lr, weight_decay=0)
        else:
            for param in model.parameters():
                param.requires_grad = True
            args.gtrans.lr = args.gtrans.lr_feat
            optimizer = optim.Adam(model.parameters(), lr=args.gtrans.lr, weight_decay=0)
            # args.gtrans.lr =0.001; args.gtrans.epochs=10
        ...
        model.eval()
        for i in range(args.gtrans.epochs):
            optimizer.zero_grad()
    ...
    def learn_graph(self, data):
        ...
        for it in tqdm(range(args.gtrans.epochs)):
    ...
    def test_time_loss(self, model, batch, mode='train'):
        args = self.args
        loss = 0
        if 'LC' in args.gtrans.loss: # label constitency
            if mode == 'eval': # random seed setting
                random.seed(args.gtrans.seed)
                np.random.seed(args.gtrans.seed)
                torch.manual_seed(args.gtrans.seed)
                torch.cuda.manual_seed(args.gtrans.seed)
            assert args.gtrans.strategy == 'dropedge'
            x_backup, edge_index_backup, edge_weight_backup = batch.x, batch.edge_index, batch.edge_weight
                # output1 = self.augment(strategy=args.gtrans.strategy, p=0.5, edge_index=edge_index, edge_weight=edge_weight)
            output1 = self.augment(strategy=args.gtrans.strategy, p=0.05, batch=adjust_batch(batch, x_backup, edge_index_backup, edge_weight_backup)) #TODO
            # if args.gtrans.strategy == 'dropnode':
            #     output1 = self.augment(strategy=args.gtrans.strategy, p=0.05, edge_index=edge_index, edge_weight=edge_weight)
            # if args.gtrans.strategy == 'rwsample':
            #     output1 = self.augment(strategy=args.gtrans.strategy, edge_index=edge_index, edge_weight=edge_weight)
            output2 = self.augment(strategy='dropedge', p=0.0, batch=adjust_batch(batch, x_backup, edge_index_backup, edge_weight_backup))
            output3 = self.augment(strategy='shuffle', batch=adjust_batch(batch, x_backup, edge_index_backup, edge_weight_backup))
            if args.gtrans.margin != -1:
                loss = inner(output1[0], output2[0]) - inner_margin(output2[0], output3[0], margin=args.gtrans.margin)
            else:
                loss = inner(output1[0], output2[0]) - inner(output2[0], output3[0])
            # import pdb; pdb.set_trace()

        if 'recon' in args.gtrans.loss: # data reconstruction
            model = self.model
            delta_feat = self.delta_feat
            feat = self.feat + delta_feat
    ...
    def pretrain_model(self, verbose=True):
        ...
        # reset_args(args)
        if args.gtrans.model == "GCN" or args.gtrans.model == "GCNSLAPS":
            save_mem = False
            model = GCN(nfeat=feat.shape[1], nhid=args.gtrans.hidden, dropout=args.gtrans.dropout, nlayers=args.gtrans.nlayers,
                        weight_decay=args.gtrans.weight_decay, with_bn=True, lr=args.gtrans.lr, save_mem=save_mem,
                        nclass=max(labels).item()+1, device=device, args=args).to(device)

        elif args.gtrans.model == "GAT":
            model = GAT(nfeat=feat.shape[1], nhid=32, heads=4, lr=args.gtrans.lr, nlayers=args.gtrans.nlayers,
                  nclass=labels.max().item() + 1, with_bn=True, weight_decay=args.gtrans.weight_decay,
                  dropout=0.0, device=device, args=args).to(device)
        elif args.gtrans.model == "GCNII":
            model = GCNII(nfeat=feat.shape[1], nhid=32, lr=args.gtrans.lr, nlayers=10,
                  nclass=labels.max().item() + 1, weight_decay=args.gtrans.weight_decay,
                  dropout=0.0, device=device, args=args).to(device)
        elif args.gtrans.model == "SAGE":
            if args.gtrans.dataset == "fb100":
                model = SAGE2(feat.shape[1], 32, max(labels).item()+1, num_layers=args.gtrans.nlayers,
                        dropout=0.0, lr=0.01, weight_decay=args.gtrans.weight_decay,
                        device=device, args=args, with_bn=args.gtrans.with_bn).to(device)
            else:
                model = SAGE(feat.shape[1], 32, max(labels).item()+1, num_layers=args.gtrans.nlayers,
                        dropout=0.0, lr=0.01, weight_decay=args.gtrans.weight_decay, device=device,
                        args=args, with_bn=args.gtrans.with_bn).to(device)
        elif args.gtrans.model == "GPR":
            model = GPRGNN(feat.shape[1], 32, max(labels).item()+1, dropout=0.0,
                    lr=0.01, weight_decay=args.gtrans.weight_decay, device=device, args=args).to(device)
        elif args.gtrans.model == "APPNP":
            if args.gtrans.dataset == 'ogb-arxiv':
                model = APPNP(nfeat=feat.shape[1], nhid=args.gtrans.hidden, dropout=0.2, nlayers=4,
                            with_bn=True, lr=0.01,
                            weight_decay=0, nclass=max(labels).item()+1, device=device, args=args).to(device)
            else:
                model = APPNP(nfeat=feat.shape[1], nhid=args.gtrans.hidden, dropout=0.5, nlayers=10,
                        weight_decay=args.gtrans.weight_decay,
                        nclass=max(labels).item()+1, device=device, args=args).to(device)
        else:
        ...
        if args.gtrans.ood:
            filename = f'saved/{args.gtrans.dataset}_{args.gtrans.model}_s{args.gtrans.seed}.pt'
            if args.gtrans.model == "GCNSLAPS":
                filename = f'saved/{args.gtrans.dataset}_GCN_s{args.gtrans.seed}.pt'
        else:
            filename = f'saved_no_ood/{args.gtrans.dataset}_{args.gtrans.model}_s{args.gtrans.seed}.pt'
        if args.gtrans.debug and osp.exists(filename):
            model.load_state_dict(torch.load(filename, map_location=self.device))
        else:
            train_iters = 500 if args.gtrans.dataset == 'ogb-arxiv' else 200
            model.fit_inductive(data_all, train_iters=train_iters, patience=500, verbose=True)
            if args.gtrans.debug:
                torch.save(model.state_dict(), filename)
        if args.gtrans.model == "GCNSLAPS":
            assert args.gtrans.debug > 0
            model.setup_dae(feat.shape[1], nhid=args.gtrans.hidden, nclass=feat.shape[1])
    ...
    def evaluate_single(self, model, output, labels, test_data, verbose=True):
        ...
        if self.args.gtrans.dataset in ['ogb-arxiv']:
            acc_test = eval_func(labels[test_data.test_mask], output[test_data.test_mask])
        elif self.args.gtrans.dataset in ['cora', 'amazon-photo', 'twitch-e', 'fb100']:
            acc_test = eval_func(labels, output)
        elif self.args.gtrans.dataset in ['elliptic']:
