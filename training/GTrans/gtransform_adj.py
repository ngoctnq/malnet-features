...
class EdgeAgent(FeatAgent):
    ...
    def learn_graph(self, data):
        ...
        feat, labels = data.x.to(self.device), data.label.to(self.device)#.squeeze()
    ...
    def sample_random_block(self, n_perturbations):
        if self.args.gtrans.existing_space:
